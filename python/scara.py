...
class Waypoint:
  def __init__(self, label, start, end=None, radius=None, angle=None):
    self.label = label
    self.start = start
    self.end = end
    self.radius = radius
  ...
  def getTrajectory(self, waypoints):
    ...
    for wp in waypoints:
      if wp.label == 'line':
        ...
        path, angles = ik.circleTrajectory(wp.start, wp.radius, wp.angle, self.L1, self.L2, self.D)
...
def deathlyHallows():
  waypoints = [
    Waypoint(label='line', start=[59, 310], end=[-41, 138]),
    Waypoint(label='line', start=[-41, 138], end=[159, 138]),
    Waypoint(label='line', start=[159, 138], end=[59, 310]),
    Waypoint(label='line', start=[59, 310], end=[59, 138]),
    Waypoint(label='circle', start=[59, 138], radius=58, angle=3*np.pi/2),
  ]
